...
export default function SchoolsPage() {
  ...
  const applyFilters = useCallback(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortColumn === "country") {
        const countryA =
          countries.find((c) => c.id === a.countryId)?.name || "";
        const countryB =
          countries.find((c) => c.id === b.countryId)?.name || "";
  ...
  useEffect(() => {
    ...
    if (sortDirectionFromUrl && ["asc", "desc"].includes(sortDirectionFromUrl))
  ...
  const getCountryName = (countryId: string) =>
    countries.find((c) => c.id === countryId)?.name || "Unknown";
  ...
  const columns = [
    {
      ...
      header: "ID",
      cell: (school: School) => (
        <span className="text-muted-foreground text-sm">{school.id}</span>
      ),
      sortable: true,
    },
    {
    ...
    },
    {
      id: "country",
      header: "Country",
      cell: (school: School) => <span>{getCountryName(school.countryId)}</span>,
      sortable: true,
    },
    {
      id: "city",
      header: "City",
      cell: (school: School) => <span>{school.city}</span>,
      sortable: true,
    },
    {
      id: "address",
      header: "Address",
      cell: (school: School) => (
        <span className="truncate block max-w-[250px]">{school.address}</span>
      ),
      sortable: false,
    },
    {
      id: "createdAt",
      header: "Created At",
      cell: (school: School) => new Date(school.createdAt).toLocaleDateString(),
      sortable: true,
    },
    {
      id: "actions",
      ...
      cell: (school: School) => (
        <div className="flex justify-end">
          <DropdownMenu key={`${school.id}-${formDrawerOpen}`}>
  ...
  return (
    <AdminLayout>
      ...
      <DataFormDrawer
        ...
        description={
          isEditMode ? "Update school details" : "Create a new school"
        }
        open={formDrawerOpen}
        onOpenChange={(open) => {
